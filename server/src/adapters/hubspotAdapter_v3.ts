...
export async function findContactByEmail(email: string): Promise<{ id: string; companyName?: string } | null> {
    ...
    try {
        ...
        const searchRequest = {
            ...
            // Request simple company name property AND associated company ID
            properties: ['company', 'associatedcompanyid'],
        };
        ...
        if (response.results && response.results.length > 0) {
            ...
            // 1. Try to get associated company ID from properties
            const companyId = contact.properties['associatedcompanyid'];

            if (companyId) {
                try {
                    // Fetch company details using the companies API
                    const company = await client.crm.companies.basicApi.getById(companyId, ['name']);
                    companyName = company.properties['name'] || undefined;
                } catch (err) {
                    console.warn(`[HubSpot] Failed to fetch company details for ID ${companyId}`, err);
                }
            }

            // 2. Fallback to text property if ID lookup fails or is missing
            if (!companyName) {
...
export async function createDeal(
    ...
    try {
        ...
        const SimplePublicObjectInputForCreate = {
            properties: {
                dealname: meetingDetails.dealName || meetingDetails.topic,
